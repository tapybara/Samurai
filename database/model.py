...
class History(Base):
    ...
    user = Column('user', String, nullable=False)               #ユーザー名
    refer = Column('refer', String, nullable=False)             #アクセス元IP
    time = Column('time', DateTime, nullable=False)             #アクセス日時
    param = Column('param', String)                             #送信内容（ON/OFF情報）

class User(Base):
    __tablename__ = 'users'
    user_id = Column('user_id', Integer, primary_key = True)    #ユーザーID
    user = Column('user', String, nullable=False)               #ユーザー名
    password = Column('password', String, nullable=False)       #パスワード
